<template>
  <div class="orders-manage-page">
    ...
    <div class="orders-block">
      <h3>筛选条件</h3>
      <div class="table-container filters-panel">
    ...
    <!-- 统计信息 -->
    <div class="orders-block stats-block">
      <h3>数据概览</h3>
    ...
    <div class="orders-block">
      <h3>订单列表</h3>
      <div class="table-container orders-list">
        ...
        <div class="order-card" v-for="order in orders" :key="order.orderNo">
          ...
          <div class="order-info">
            ...
            <div class="info-row">
              <div class="info-item">
                ...
                <span class="card-type" :class="order.cardType">
                  {{ order.cardType === 'time' ? '时间卡' : '次数卡' }}
                </span>
                <span class="card-spec">{{ order.cardSpec }}</span>
              </div>
              <div class="info-item">
          ...
          <div class="order-actions">
            ...
            <button
              ...
              class="btn-success"
              @click="updateOrderStatus(order.orderNo, 'completed')"
            >
            ...
            <button
              v-if="order.status === 'pending'"
              class="btn-danger"
              @click="updateOrderStatus(order.orderNo, 'failed')"
            >
        ...
        <div class="modal-body" v-if="selectedOrder">
          ...
          <div class="detail-section">
            ...
            <div class="detail-grid">
              <div class="detail-item">
                <label>卡密类型：</label>
                <span class="card-type-badge" :class="selectedOrder.cardType">
                  {{ selectedOrder.cardType === 'time' ? '时间卡' : '次数卡' }}
                </span>
              </div>
              <div class="detail-item">
                <label>规格：</label>
                <span>{{ selectedOrder.cardSpec }}</span>
              </div>
              <div class="detail-item">
                <label>数量：</label>
              ...
              <div class="detail-item">
                <label>总价：</label>
                <span class="total-price">¥{{ selectedOrder.totalPrice }}</span>
              </div>
              <div class="detail-item">
                <label>支付方式：</label>
          ...
          <div class="detail-section" v-if="selectedOrder.cardKeys && selectedOrder.cardKeys.length > 0">
            <h4>卡密信息</h4>
            <div class="card-keys">
              <div v-for="(key, index) in selectedOrder.cardKeys" :key="index" class="card-key-item">
...
const getOrders = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // 处理数据，添加 cardKeys 数组转换
      orders.value = rawList.map(order => ({
        ...order,
        cardKeys: order.cardKeys ? order.cardKeys.split(',') : []
      }))
...
const updateOrderStatus = async (orderId, status, remark = '') => {
  try {
    await ElMessageBox.confirm(`确定要将订单状态更新为"${getStatusText(status)}"吗？`, '确认操作', { type: 'warning' })
  } catch { return }
...
<style scoped>
/* 与「卡密定价管理」页统一的布局与色板：主色 #2563eb、危险 #dc2626、卡片圆角 8px、轻阴影 */
.orders-manage-page {
  padding: 1rem;
  width: 100%;
...
.spin {
  animation: spin-orders 0.9s linear infinite;
}
...
.order-actions .btn-primary.btn-sm {
